...
const UploadEmotionImages = () => {
  ...
  const handleSubmitEmotion = async (emotion) => {
    try {
      const file = images[emotion];
      if (!file) {
        alert(`กรุณาเลือกภาพสำหรับอารมณ์ ${emotionLabels[emotion]}`);
        return;
      }
      ...
      const { data, error } = await supabase.storage
        ...
        .upload(filePath, file); // อัปโหลดไฟล์ไปยัง Supabase
      ...
      const { error: insertError } = await supabase.from('img_student').insert([
        {
          ...
          [`img_${emotion.toLowerCase()}`]: publicUrl, // บันทึกรูปภาพตามอารมณ์นั้นๆ
          uploadimg_date: new Date().toISOString(),
        },
      ]);

      if (insertError) {
        throw new Error(`Failed to insert record for ${emotion}: ${insertError.message}`); // แจ้งเตือนหากการบันทึกล้มเหลว
      }

      alert(`${emotionLabels[emotion]} รูปภาพอัปโหลดและบันทึกสำเร็จ!`); // เปลี่ยนข้อความแจ้งเตือนเป็นภาษาไทย

      // รีเซ็ตค่า image ของอารมณ์ที่อัปโหลด
      setImages({
        ...images,
        [emotion]: null,
      });
            ...
            <div className="grid grid-cols-4 gap-12 mb-8">
              {emotions.slice(0, 4).map((emotion) => (
                ...
                >
                  ...
                  <div className="w-48 h-48 bg-gray-200 mb-3 flex justify-center items-center overflow-hidden">
                    {images[emotion] ? (
                      <img
                      ...
                      <div className="text-center text-gray-500">เลือกภาพ</div> // เพิ่มข้อความแนะนำเมื่อยังไม่มีภาพ
                    )}
                  </div>
                  <input
                    type="file"
                  ...
                  <button
                    type="button"
                    onClick={() => handleSubmitEmotion(emotion)}
                    className="px-4 py-2 bg-green-400 text-white font-semibold rounded-lg mt-3 hover:bg-green-200 transition-colors"
                    style={{ marginTop: 'auto' }} // เพิ่ม marginTop:auto เพื่อให้ปุ่มอยู่ในตำแหน่งที่ถูกต้อง
                  >
                    บันทึก
                  </button>
                </div>
